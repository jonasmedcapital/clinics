class Operations::Products::Clinics::Calculations::UpdateYearlyCalculationService
  ...
  def monthly_calculations
    ...
    if Time.zone.now.year == @calculation.year
      end_month = Time.zone.now.month
    else
      end_month = 12
    end
    @monthly_calculations = Operations::Products::Clinics::MonthlyCalculationRepository.find_year_month(@clinic.id, @calculation.year, start_month, end_month)
  end
  ...
  def update_yearly_calculation
    ...
    @yearly_calculation = Operations::Products::Clinics::YearlyCalculationRepository.find_year(@clinic.id, @calculation.year).first
